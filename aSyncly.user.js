...
function UpdateLocalStorage(json) {
	...
	const keys = Object.keys(localStorage).filter(key => key.startsWith('Doc_') | key.startsWith('PNW_'));
...
async function UpdateDossier(hash, token) {
	...
	const text = JSON.stringify(Object.entries(localStorage).filter(([key, _]) => key.startsWith('Doc_') | key.startsWith('PNW_')).sort((x, y) => x[0] > y[0] ? 1 : y[0] < x[0] ? -1 : 0).map(([key, value]) => [key, value.replaceAll('"', '!!')]));
