...
let token = (() => {
	const inputTag = Array.from(document.getElementsByTagName('input')).filter(x => x.name == 'token')[0];
	if (inputTag) {
...
function SetUpButton() {
	const divTag = (() => {
		...
		if (token) {
			...
			const pTag = Array.from(document.getElementsByTagName('a')).filter(x => x.href == location.href)[0].parentElement;
			pTag.previousElementSibling.remove();
			pTag.nextElementSibling.remove();
			pTag.parentElement.insertBefore(divTag, pTag);
			pTag.remove();
		}
		return divTag;
	})();
	...
	while (isPlaying) {
		if (!token) {
			token = await (async () => {
				...
				const inputTag = Array.from(doc.getElementsByTagName('input')).filter(x => x.name == 'token')[0];
				if (inputTag) {
					return inputTag.value;
				}
				if (!Array.from(doc.getElementsByTagName('a')).filter(x => x.href == location.href)[0]) {
					location.reload();
				}
			})();
		}
...
async function Played() {
	...
	// Check if Successful...
	const num = doc.getElementsByClassName('nationtable').length;
	// Game was played.
	if (num > 1) {
		const inputTag = Array.from(doc.getElementsByTagName('input')).filter(x => x.name == 'token')[0];
		if (inputTag) {
	...
	// Play button on page, but no game was played.
	if (num > 0) {
		// This shouldn't happen here?
		console.debug('THIS SITUATION HAPPENED!');
...
function Sound() {
    document.querySelector('div#DIV button#PLAY.btn').click();
    /*
